...
export default function VaultCard({ vault, onAction, animDelay=0 }) {
  ...
  const status=Number(vault.status), deadline=Number(vault.deadline), interval=Number(vault.intervalSeconds);
  ...
  return (
    <div className="card fade-up" style={{marginBottom:14,animationDelay:`${animDelay}s`,opacity:0}}>
      <div className="row-between" style={{marginBottom:16}}>
        <div>
          <div style={{fontFamily:"var(--font-display)",fontSize:15,fontWeight:700,letterSpacing:".06em",marginBottom:4}}>{vault.name||"Unnamed Vault"}</div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <span className="label">{vault.id?`VLT-${String(vault.id).padStart(4,"0")}`:""}</span>
      ...
      {status===0&&(<>
        ...
        {isExpired?<div style={{color:"var(--red)",fontFamily:"var(--font-mono)",fontSize:13,marginBottom:16}}>Deadline passed. Heirs may now initiate a claim.</div>:<Countdown deadlineTs={deadline} urgent={isUrgent}/>}
        <div className="progress-track" style={{marginBottom:20}}><div className={`progress-fill ${pgClass}`} style={{width:`${pct}%`}}/></div>
      </>)}
      {status===1&&<div className="alert alert-danger" style={{marginBottom:16}}>Vault is claimable. Awaiting heir claim and co-signer approval.</div>}
