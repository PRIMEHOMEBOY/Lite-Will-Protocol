...
export default function VaultCard({vault,onAction,animDelay=0}){
  ...
  const grace=Number(vault.gracePeriodSeconds||0);
  ...
  const isInGrace=isExpired&&now<(deadline+grace);
  const isClaimable=isExpired&&now>=(deadline+grace);
    ...
    <div className="card fade-up" style={{marginBottom:14,animationDelay:`${animDelay}s`,opacity:0}}>
      <div className="row-between" style={{marginBottom:16}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
          <span className={`badge ${STATUS_BADGES[status]||"badge-active"}`}>
            {isInGrace?"⏳ GRACE PERIOD":isUrgent?"⚡ URGENT":STATUS_LABELS[status]||"UNKNOWN"}
          </span>
      ...
      {status===0&&!isExpired&&(<>
        <div className="label" style={{marginBottom:8}}>Time until release</div>
      ...
      {status===0&&isInGrace&&(
        <div className="alert alert-warn" style={{marginBottom:16}}>
          ⏳ Deadline passed. Grace period active — heirs can claim in {Math.ceil(((deadline+grace)-now)/86400)} more day{Math.ceil(((deadline+grace)-now)/86400)!==1?"s":""}.
        </div>
      )}
      {status===0&&isClaimable&&(
        <div className="alert alert-danger" style={{marginBottom:16}}>
          Grace period over. Heirs can now trigger a claim.
        </div>
      )}
