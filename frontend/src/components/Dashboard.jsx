...
function StatCard({ label, value, color, delay }) {
  return (
    <div className="card fade-up" style={{animationDelay:`${delay}s`,opacity:0}}>
      ...
      <div style={{fontFamily:"var(--font-display)",fontSize:38,fontWeight:900,color,lineHeight:1,textShadow:`0 0 20px ${color}50`}}>{value}</div>
    </div>
  );
}
...
export default function Dashboard({ onTabChange, onAction }) {
  ...
  if (!isConnected) return (
    <div className="empty-state fade-in">
      ...
      <div style={{color:"var(--text-muted)",fontFamily:"var(--font-mono)",fontSize:12,marginBottom:28}}>Connect to LiteForge Testnet to access your vaults</div>
    </div>
  );
  return (
    <div className="fade-in">
      <div className="grid-3" style={{marginBottom:36}}>
        ...
        <StatCard label="Total on Chain" value={String(total)}   color="var(--red)"   delay={0.16}/>
      </div>
      ...
      {ownerIds.length===0?(
        <div className="empty-state" style={{padding:"40px 0"}}>
          ...
          <div style={{color:"var(--text-muted)",fontFamily:"var(--font-mono)",fontSize:11,marginBottom:20}}>Create your first Dead Man's Switch</div>
