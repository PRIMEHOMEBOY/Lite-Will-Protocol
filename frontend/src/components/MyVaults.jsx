...
function VaultLoader({id,onAction,index,filter}){
  ...
  if(!d)return null;
  const vault={id,owner:d[1],name:d[2],secretType:d[5],intervalSeconds:d[6],lastCheckIn:d[7],deadline:d[8],coSigner:d[9],status:d[10],createdAt:d[11]};
  ...
  return(
    <div className="fade-in">
      <div style={{display:"flex",gap:8,marginBottom:28,flexWrap:"wrap"}}>
        ...
        <div style={{marginLeft:"auto",fontFamily:"var(--font-mono)",fontSize:11,color:"var(--text-muted)",alignSelf:"center"}}>{ids.length} vault{ids.length!==1?"s":""}</div>
      </div>
      {isLoading&&<div style={{textAlign:"center",padding:"40px 0",color:"var(--text-muted)",fontFamily:"var(--font-mono)",fontSize:12}}><span className="spinner" style={{marginRight:10}}/>Loading vaults...</div>}
