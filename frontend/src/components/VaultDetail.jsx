...
export default function VaultDetail({vaultId,onClose}){
  ...
  if(!d)return(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}><div style={{textAlign:"center",padding:"40px 0",color:"var(--text-muted)",fontFamily:"var(--font-mono)"}}><span className="spinner"/> Loading...</div></div></div>);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:560,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{marginBottom:24}}>
          ...
          <div style={{fontFamily:"var(--font-display)",fontSize:20,fontWeight:700,marginBottom:8}}>{vault.name}</div>
        ...
        {heirs.map((h,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:"rgba(255,255,255,.02)",border:"1px solid var(--border)",marginBottom:8}}>
            <div><div style={{fontFamily:"var(--font-mono)",fontSize:12}}>{h.label||`Heir ${i+1}`}</div><div style={{fontFamily:"var(--font-mono)",fontSize:10,color:"var(--text-muted)"}}>{short(h.wallet)}</div></div>
            <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontFamily:"var(--font-display)",fontSize:18,fontWeight:700,color:"var(--red)"}}>{(Number(h.shareBps)/100).toFixed(0)}%</span>{h.claimed&&<span className="badge badge-done">Claimed</span>}</div>
          </div>
        ))}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {status===0&&isExpired&&<button className="btn btn-danger btn-full" onClick={()=>triggerClaimable(vaultId)} disabled={isTrigger}>{isTrigger?<><span className="spinner"/> Confirming...</>:"⚡ Trigger Claimable State"}</button>}
